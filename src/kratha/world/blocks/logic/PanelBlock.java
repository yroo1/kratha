        ...
        public void drawSelect(){
            if(KrathaLogic.logger instanceof PanelLogger l){
                if(reqChip1>0&&items.get(l.chip1)<reqChip1){
                    drawPlaceText((Core.bundle.get("item."+l.chip1.name+".name")+" : "+items.get(l.chip1)+"/"+reqChip1),x,y,true);
                }
                if(reqChip2>0&&items.get(l.chip2)<reqChip2){
                    drawPlaceText((Core.bundle.get("item."+l.chip2.name+".name")+" : "+items.get(l.chip2)+"/"+reqChip2),x,y,true);
                }
                if(reqChip3>0&&items.get(l.chip3)<reqChip3){
                    drawPlaceText((Core.bundle.get("item."+l.chip3.name+".name")+" : "+items.get(l.chip3)+"/"+reqChip3),x,y,true);
                }
                if(reqChip4>0&&items.get(l.chip4)<reqChip4){
                    drawPlaceText((Core.bundle.get("item."+l.chip4.name+".name")+" : "+items.get(l.chip4)+"/"+reqChip4),x,y,true);
                }
            }
        }
